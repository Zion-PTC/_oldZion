...
export class AudiusApi {
  ...
  async fetchAudius<T>(
    type,
    query
  ): Promise<T extends 'streamTrack' ? { url; stream } : { data } | ZionError> {
    return new Promise(async (resolve, reject) => {
      /*write code here*/
      let fetchAttrObj = await this.setUrlToFetch(type, query);
      let res: Response = await fetch(fetchAttrObj.url, fetchAttrObj.options);
      if (type !== 'streamTrack') {
  ...
  async createHashtagPlaylist(query) {
    ...
    let createdPlaylist;
    let host = await this.getFastestHost();
    let hashtag: Response = await fetch(
    ...
    if (titles) {
      ...
      for (let titleOfTrackUsingHashtag of titles) {
        let res: { data } | ZionError = await this.fetchAudius<'searchTrack'>(
          'searchTrack',
