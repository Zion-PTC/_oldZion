 ...
 * ```
 */
export declare type ClassType<X, T = number> = new (
...
export type ComplexMixin<X> = <TBase extends Constructor>(
  Base: TBase,
  id: number
) => {
  new (...arg: any[]): X;
...
export type InterfaceOfClass<T> = FlattenInterface<
  InstanceType<T extends Constructor ? T : never>
>;
...
> &
  InterfaceOfClass<Superclass extends Constructor ? Superclass : never>;

export type ConstrOfInterfaceOfBoth<
  MixinType extends Mixin,
  Superclass extends Constructor
> = GCtor<InterfaceOfBoth<MixinType, Superclass>>;
