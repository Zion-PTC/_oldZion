...
module.exports.logDeploymentGasLimit = async (contract, name) => {
  return console.log("%d of gas is required to deploy the contract:", parseInt(contract.deployTransaction.gasLimit._hex), name);
}

module.exports.emitsEventOnDeploy = async (contract, event) => {
...
module.exports.hashTx = async ({ contract, functionName, arguments }) => {
  return contract.interface.encodeFunctionData(functionName, arguments)
}

module.exports.calculateProposalId = async ({ targets, values, calldatas, description }) => {
  let types = [
    "address[]",
    "uint256[]",
    "bytes[]",
  ...
  let abiEncodedProposal = await this.abi.encode(types, [targets, values, calldatas, description])
  let hashedProposal = await ethers.utils.keccak256(abiEncodedProposal)
  return ethers.BigNumber.from(hashedProposal).toString()
}

module.exports.calculateCallDatas = async (txs) => {
  let array = []
...
module.exports.zionUtils = {
  ...
  logDeploymentGasCost : async (contract, message) => {
    return console.log(`${message}`, parseInt(contract.deployTransaction.gasLimit._hex));
  }
}
...
module.exports.governance = {
  deployProposal: async (governor, { targets, values, txs, description }) => {
    ...
    let proposalObj = { targets, values, calldatas, description: hashedDescription }
  ...
  precondition1: async (usdc, zionDao, employee,) => {
    ...
    let { proposal, proposalId, proposalObj: proposalObj_ } = await this.governance.deployProposal(zionDao, { ...proposalObj })
    return {
      proposal,
      proposalId,
      proposalObj_
  ...
  precondition2: async (usdc, zionDao, employee, holder1, holder2) => {
    let { proposalId, proposalObj_: proposalObj } = await this.governance.precondition1(usdc, zionDao, employee)
    await this.zionUtils.getBlockNumber()
  ...
  precondition3: async (usdc, zionDao, employee, holder1, holder2, holder3) => {
    let { proposalId, proposalObj } = await this.governance.precondition2(usdc, zionDao, employee, holder1, holder2)
