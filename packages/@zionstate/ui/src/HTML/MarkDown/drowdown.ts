...
function markdown(src: string) {
  ...
  function replace(
    rex: RegExp,
    fn: ((substring: string, ...args: any[]) => string) | string
  ) {
    if (typeof fn !== "string") src = src.replace(rex, fn);
    if (typeof fn === "string") src.replace(rex, fn);
  }
  ...
  function highlight(src: string): any {
    return src.replace(
      rx_highlight,
