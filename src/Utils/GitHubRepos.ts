...
type ISROOT = true | string;
type HASCONF = true | string;
type ISWORKING = string;
type ISCOMMIT = string | undefined;
type DEPS = string[] | number;
...
const mapped = array.map(async (pacchetto) => {
  if (pacchetto[1] !== typeof "string") throw "wrong argument";
  if (!Array.isArray(pacchetto[5])) throw "not an array";
...
const res = (await Promise.all(mapped)) as any as {
  name?: NAME;
  packageName?: PACKAGENAME;
  isRoot?: ISROOT;
  hasTypesInConfig?: HASCONF;
  isWorking?: ISWORKING | { wins: number; fails: number };
