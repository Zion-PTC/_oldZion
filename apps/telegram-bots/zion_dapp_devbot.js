...
// let createSingleLineCallbackButtons =
//   zionTelegraf.createSingleLineCallbackButtons;
...
let arrayOfWords2 = {
  listOfWords: [
    ["bot", "robot", "Bot"],
    ["bastard", "bastardo", "infame", "stronzo"],
  ],
  messages: [
    { sec: 0, text: "ehy" },
    { sec: 1, text: "you" },
    ...
    ctx.reply(NEW_MEMBER_WELCOME_MESSAGE(newMember, ctx), {
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "Chat with me, bitch",
              url: "https://t.me/zion_dapp_bot",
            },
          ],
        ],
      },
...
bot.start((ctx) =>
  createSendMessage(
    ...
    ctx,
    WELCOME_MESS(ctx),
    createSingleLineCallbackButtons(mainMenuLines)
  )
);
bot.command("zion_tokenomics", (ctx) =>
  createSendMessage(
    bot,
    ctx,
    WELCOME_MESS,
    createSingleLineCallbackButtons(tokenMenuLines)
  )
);
...
bot.command("ciao", (ctx) =>
  ctx.reply(
    "Ecco",
    zionTelegraf.createReplyMarkupKeyboard([[{ text: "prova" }]])
  )
);

bot.command("test", (ctx) =>
  ctx.reply("2", zionTelegraf.createReplyMarkupKeyboard([[{ text: "prova2" }]]))
);

const COMMAND_LIST = ["/start", "/zion_tokenomics"];

const backToCommandsLines = [[["« Back to Main Menu", "menu"]]];
...
//   callback: createReplyMarkup(createSingleLineCallbackButtons(introMenuLines))
// }
...
const commandsObj = {
  ...
  title: "commands",
  mess:
    "Here you can find the list of avalaible commands\n(type the command or simply click on it):" +
    commands,
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToCommandsLines)
  ),
};

const backToTokenLines = [
  [
    ["« Back to Token Menu", "tokenomics"],
    ["« Back to Main Menu", "menu"],
  ],
];
...
const incomeObj = {
  ...
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToTokenLines)
  ),
};
...
const tokenomicsObj = {
  ...
  title: "token_dist",
  mess:
    "The distribution of tokens is initially set as following:\n10% Creators,\n25% Team,\n25% Dao,\n30% Investors,\n10% Stake." +
    DAOVOTE,
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToTokenLines)
  ),
...
const supplyObj = {
  ...
  title: "supply",
  mess:
    "The supply is set to be 1B tokens, and it will be inflationary. Inflation will be used to reward digital and physical nodes. The inflation rate is not yet been established." +
    DAOVOTE,
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToTokenLines)
  ),
...
const backToTokensLines = [
  [
    [`« Back to DAO Tokens Menu`, `tokens`],
    [`« Back to Main Menu`, `menu`],
  ],
];
...
const zionTokenObj = {
  ...
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToTokensLines)
  ),
};
...
const backToMainMenu = [[["« Back To Main Menu", "menu"]]];
...
const webPageObj = {
  ...
  callback: createReplyMarkup(
    createSingleLineCallbackButtons(backToLinksLines)
  ),
};
