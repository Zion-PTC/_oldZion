...
export class Audius {
  ...
  async setUrlToFetch(type, query) {
    ...
    let getUserRepost = {
      url:
        host + VERSION + USERS + STRING + REPOSTS + APPNAME,
      options: { method, headers },
    };
    let searchPlaylist = {
      url:
    ...
    let searchTag = {
      url:
        host +
        TAG_SEARC_URL_PREFIX +
        query +
        TAG_SEARCH_URL_SUFFIX,
      // options: { method, headers }
    };
    let streamTrack = {
      url:
        host + VERSION + TRACKS + QUERY + STREAM + APPNAME,
      options: { method },
    };
    let userTracks = {
      url:
        host + VERSION + USERS + QUERY + TRACKS + APPNAME,
      options: { method, headers },
    };
    let trendingTracks = {
  ...
  async fetchAudius(type, query) {
    return new Promise(async (resolve, reject) => {
      /*write code here*/
      let fetchAttrObj = await this.setUrlToFetch(
        type,
        query
      );
      let res = await fetch(
      ...
      if (type === 'streamTrack') {
        resolve({
          url: res[Object.getOwnPropertySymbols(res)[1]]
            .url,
          stream: res,
        });
      }
  ...
  async createHashtagPlaylist(query) {
    ...
    if (titles) {
      ...
      tracks
        ? (createdPlaylist = this.composePlaylist(tracks))
        : {};
    }
