      ...
      if (typeof size === 'number' && Array.isArray(array)) {
        ...
        rej(
          typeof size !== 'number'
            ...
            : 'The second argument shall be an array'
        );
        return false;
      }
    });
...
export let zionTelegraf = {
  createAction: async (/* fatto */ { bot, title, mess, callback }) => {
  ...
  createCommandList: (/* NON SERVE */ array) => {
    let string = ``;
    if (array.length >= 1)
      array.forEach(command => (string = string + `\n` + command));
    else string = string;
    return string;
  },
  createSendMessage: (bot, ctx, mess, array) => {
    bot.telegram.sendMessage(
      ctx.chat.id,
      mess,
      zionTelegraf.createReplyInlineKeyboard(array)
    );
  },
  createReplyInlineKeyboard: (/* fatto */ array) => {
  ...
  createKeyboard: (/* NON SERVE */ array) => {
  ...
  createBotResponseForArrayOfWords: async (
    ctx,
    { /* fatto */ listOfWords, messages, type, limit }
  ) => {
  ...
  timedOutMessage: async (sec, mess, ctx) => {
    /* fatto */
    if (sec === 0) {
  ...
  replyToMessage: (ctx, mess) => {
    ctx.reply(mess, { reply_to_message_id: ctx.message.message_id });
  },
  ctx: {
    botInfo: ctx => {
      return ctx.botInfo;
  ...
  askConfirmationMess: async ctx => {
    ...
    return (
      'Ciao ' +
      user +
      '. Hai richiesto di visualizzare le task della chat' +
      chatName +
      '. Confermi? Si o No'
    );
  },
  ...
  botReply: async (ctx, text, keyboard) => {
    /* fatto */
    ctx.telegram.sendChatAction(ctx.chat.id, 'typing');
