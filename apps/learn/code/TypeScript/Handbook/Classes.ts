class ClassMemebers {
  ...
  static readonly() {
    console.log(
      `I campi possono essere prefissati con un modificatore readonly. Questo previene che l'assegnamento al campo fuori dal costruttore.`
    );
    class Greeter {
  ...
  static constructors1() {
    console.log(
      'I costruttori delle classi sono molto simili alle funzioni. Si possono aggiungere dei parametri con ogni tipo di annotazione, valori di default e overloads:'
    );
    class Point {
      x: number;
      y: number;
  ...
  static constructor3() {
    console.log(
      'Ci sono solo alcune differenze fra la firma di un costruttore di classe e la firma di una funzione'
    );
    console.log(
      '•I costruttori non possono avere parametri di tipo - questi appartengono alla dichiarazione esterna della classe, di cui parleremo in seguito'
    );
    console.log(
      `•I costruttori non possono avere un'annotazione di tipo return - il tipo di classe istanziata è sempre il ritorno della funzione.`
    );
  }
  static constructors4() {
    console.log('SUPER CALLS');
    console.log(
      'Come in Javascript, se si ha una classe base, dovremo chiamare super(); nel corpo del tuo costruttore prima di usare membri this:'
    );
    class Base {
      k = 4;
    }
    ...
    console.log(
      'Dimenticarsi di chiamere super è un errore semplice da fare in Javascript, ma TS ti dirà quando è necessario.'
    );
  }
  static methods1() {
    console.log('METHODS');
    console.log(
      'Le proprietà funzione in una classe è chiamato metodo. I metodi possono usare tutti gli stessi tipi di annotazione come le funzioni e i costruttori:'
    );
    class Point {
      x = 10;
      y = 10;
    ...
    console.log(
      'Oltre che alle annotazioni standard tipo, TS non aggiunge altro ai metodi'
    );
    console.log(
      'Da notare che nel corpo di un metodo, è obbligatorio accedere ai campi ed altri metodi via this. Un nome non qualificato nel corpo di un metodo farà sempre riferimento a qualcosa nello scope in cui è incluso'
    );
    function esempio() {
      let x: number = 0;
      class C {
  ...
  static getterSetters1() {
    ...
    console.log(
      'Notare che un campo che utilizza get/set senza una logica aggiuntiva vengono usati raramente in JavaScript. Va bene esporre i campi se non hai bisogno di aggiungere della logica duarente le operazioni di get e set.'
    );
    console.log(
      'Typescript ha sempre delle regole speciali per dedurre gli accessori:'
    );
    console.log(
      '•se esiste un get ma non esiste un set, la proprietà è automaticamente readonly'
    );
    console.log(
      '•se il tipo del parametro del setter non è specificato, lo dedurrà dal tipo del ritorno del getter.'
    );
    console.log(
      '•I membri dei getters e dei setters devono avere la stessa visibilità'
    );
    console.log(
      'A partire da TS 4.3, è possibile avere accessori con tipi diversi per getting e setting.'
    );
    function ex2() {
      class Thing {
        _size = 0;
